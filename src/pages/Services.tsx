...
const Services = () => {
  const { get } = useSiteContent("services");

  const heroBadge = get("hero", "badge", "Our Services");
  const heroLine1 = get("hero", "heading_line1", "Full Spectrum");
  const heroLine2 = get("hero", "heading_line2", "CA Services");
  const heroDesc = get("hero", "description", "Comprehensive chartered accountancy and financial consulting services tailored for businesses, startups, MSMEs, corporates, individuals, HNIs, and NRIs.");
  const processBadge = get("process", "badge", "How We Work");
  const processHeading = get("process", "heading", "Our Process");
  const processDesc = get("process", "description", "A simple, transparent approach to delivering exceptional results");
  const ctaHeading = get("cta", "heading", "Need Help with Any of These Services?");
  const ctaDesc = get("cta", "description", "Our team of expert chartered accountants is ready to assist you. Get in touch for a free consultation.");
  ...
  try {
    const raw = get("process", "steps", "");
    if (raw) { const parsed = JSON.parse(raw); if (Array.isArray(parsed)) processSteps = parsed; }
  } catch {}

  return (
    <Layout>
      <SEO title="CA Services in Ahmedabad – Tax, GST, Audit & Advisory" description="70+ professional CA services by best chartered accountants in Ahmedabad & Gujarat." keywords="best CA services Ahmedabad, chartered accountant services Gujarat" canonicalUrl="/services" />
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl">
            ...
            <h1 className="text-4xl md:text-6xl font-display font-bold text-white mb-6 leading-tight">
              {heroLine1}<br /><span className="text-gold">{heroLine2}</span>
            </h1>
            <p className="text-white/80 text-xl leading-relaxed mb-8 max-w-2xl">{heroDesc}</p>
            <div className="flex flex-wrap gap-4">
              <Link to="/contact" className="bg-gold hover:bg-gold/90 text-navy px-8 py-4 rounded-xl font-semibold inline-flex items-center gap-3 transition-all hover:scale-105 shadow-lg">
                Get Started <ArrowRight className="w-5 h-5" />
              </Link>
              <a href="tel:+919825046598" className="bg-white/10 backdrop-blur-sm border border-white/20 text-white px-8 py-4 rounded-xl font-semibold inline-flex items-center gap-3 hover:bg-white/20 transition-all">
                <Phone className="w-5 h-5" /> +91 98250 46598
              </a>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-display font-bold text-navy mb-4">Explore Our Services</h2>
            <p className="text-grey max-w-2xl mx-auto text-lg">Choose from our comprehensive range of professional services</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {serviceCategories.map((category, index) => (
              <AnimatedCard key={index} delay={index * 50}>
                <Link to={category.link} className="bg-white rounded-2xl border border-border p-8 hover:shadow-2xl transition-all duration-500 group block h-full hover:-translate-y-2">
                  <div className={cn("w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all duration-300 bg-gradient-to-br", category.color, "group-hover:scale-110")}>
                    <category.icon className="w-8 h-8 text-navy" />
                  </div>
                  <h3 className="text-xl font-display font-bold text-navy mb-4 group-hover:text-gold transition-colors">{category.title}</h3>
      ...
      <section className="py-20 bg-navy relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            <span className="text-gold font-medium text-sm tracking-wider uppercase mb-4 block">{processBadge}</span>
            <h2 className="text-3xl md:text-5xl font-display font-bold text-white mb-4">{processHeading}</h2>
            <p className="text-white/70 max-w-2xl mx-auto text-lg">{processDesc}</p>
          </div>
              ...
              <AnimatedCard key={index} delay={index * 100}>
                <div className="relative group">
                  <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20 hover:bg-white/20 transition-all duration-300">
                    <div className="text-5xl font-display font-bold text-gold/30 mb-4 group-hover:text-gold transition-colors">{item.step}</div>
      ...
      <section className="py-20 bg-gradient-to-br from-cream/50 to-background">
        <div className="container mx-auto px-4">
          <div className="bg-white rounded-3xl p-12 shadow-xl border border-border text-center max-w-4xl mx-auto">
            ...
            <h2 className="text-3xl md:text-4xl font-display font-bold text-navy mb-4">{ctaHeading}</h2>
            <p className="text-grey mb-8 max-w-2xl mx-auto text-lg">{ctaDesc}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/contact" className="bg-navy hover:bg-navy/90 text-white px-10 py-5 rounded-xl font-bold text-lg inline-flex items-center justify-center gap-3 transition-all hover:scale-105">
                Book Consultation <ArrowRight className="w-5 h-5" />
              </Link>
              <a href="tel:+919825046598" className="bg-gold hover:bg-gold/90 text-navy px-10 py-5 rounded-xl font-bold text-lg inline-flex items-center justify-center gap-3 transition-all hover:scale-105">
