    ...
    <footer className="bg-navy text-white">
        ...
        <div className="container mx-auto px-4 py-10">
          ...
          <div className="flex flex-wrap gap-2">
            {popularServices.map((service) => (
              <Link key={service.path} to={service.path} className="text-white/60 hover:text-cream hover:bg-white/10 px-3 py-1.5 rounded-lg text-sm transition-all border border-white/5 hover:border-white/20">
                {service.name}
              </Link>
            ))}
          </div>
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          <div className="lg:col-span-2">
            ...
            <p className="text-white/70 text-sm leading-relaxed mb-6">{companyDescription}</p>
            <div className="flex gap-4">
              <a href={linkedinUrl} aria-label="LinkedIn" className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-cream hover:text-navy transition-all">
                <Linkedin className="w-5 h-5" />
              </a>
              <a href={facebookUrl} aria-label="Facebook" className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-cream hover:text-navy transition-all">
                <Facebook className="w-5 h-5" />
              </a>
              <a href={twitterUrl} aria-label="Twitter" className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-cream hover:text-navy transition-all">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {serviceCategories.map((service) => (
                <li key={service.path}>
                  <Link to={service.path} className="text-white/70 hover:text-cream transition-colors text-sm">{service.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={link.path}>
                  <Link to={link.path} className="text-white/70 hover:text-cream transition-colors text-sm">{link.name}</Link>
                </li>
              ))}
            </ul>
            ...
            <ul className="space-y-2.5">
              <li><Link to="/branches" className="text-white/70 hover:text-cream transition-colors text-sm">Best CA in Ahmedabad</Link></li>
              <li><Link to="/branches" className="text-white/70 hover:text-cream transition-colors text-sm">Best CA in Mehsana</Link></li>
              <li><Link to="/services" className="text-white/70 hover:text-cream transition-colors text-sm">Chartered Accountants Gujarat</Link></li>
              <li><Link to="/services/taxation/nri-taxation" className="text-white/70 hover:text-cream transition-colors text-sm">NRI Tax Services India</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                ...
                <div>
                  <a href={`tel:${phone1.replace(/\s/g, '')}`} className="text-white/70 hover:text-cream transition-colors text-sm block">{phone1}</a>
                  <a href={`tel:${phone2.replace(/[\s–]/g, '')}`} className="text-white/70 hover:text-cream transition-colors text-sm block">{phone2}</a>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <Mail className="w-5 h-5 text-cream flex-shrink-0 mt-0.5" />
                <a href={`mailto:${email}`} className="text-white/70 hover:text-cream transition-colors text-sm">{email}</a>
              </li>
              <li className="flex items-start gap-3">
      ...
      <div className="border-t border-white/10">
        <div className="container mx-auto px-4 py-8">
          <p className="text-white/40 text-xs leading-relaxed text-center max-w-4xl mx-auto" dangerouslySetInnerHTML={{ __html: seoText }} />
        </div>
      </div>
