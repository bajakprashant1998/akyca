...
export const ClientLogos = () => {
  ...
  return (
    <section ref={ref} className="py-10 sm:py-12 md:py-16 bg-navy relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-8 sm:mb-10 md:mb-12">
          {certifications.map((cert, index) => (
            <div
              key={index}
              className={`flex flex-col items-center text-center p-3 sm:p-4 md:p-6 bg-white/5 rounded-xl md:rounded-2xl border border-white/10 hover:border-gold/30 hover:bg-white/10 transition-all duration-500 group ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <div className="w-9 h-9 sm:w-11 sm:h-11 md:w-14 md:h-14 bg-gold/10 rounded-lg sm:rounded-xl flex items-center justify-center mb-2 sm:mb-3 group-hover:scale-110 transition-transform">
                <cert.icon className="w-4 h-4 sm:w-5 sm:h-5 md:w-7 md:h-7 text-gold" />
              </div>
              <h4 className="text-cream font-semibold text-xs sm:text-sm md:text-base mb-0.5">{cert.title}</h4>
              <p className="text-white/50 text-[10px] sm:text-xs md:text-sm">{cert.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className={`text-center transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <p className="text-white/50 text-[10px] sm:text-xs md:text-sm mb-3 sm:mb-6 uppercase tracking-wider font-medium">
          ...
          <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 md:gap-3">
            {industries.map((industry, index) => (
              <span
                key={index}
                className="px-2.5 sm:px-4 py-1 sm:py-2 bg-white/5 text-cream text-[10px] sm:text-xs md:text-sm rounded-full border border-white/10 hover:border-gold/30 hover:bg-white/10 transition-all duration-300 cursor-default"
              >
                {industry}
              </span>
            ))}
