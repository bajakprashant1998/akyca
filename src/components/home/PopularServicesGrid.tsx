        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {popularLinks.map((item, index) => (
            <Link
              key={item.path}
              to={item.path}
              className={`group bg-card rounded-2xl p-6 border border-border hover:shadow-xl hover:-translate-y-1 transition-all duration-500 block ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
              }`}
              style={{ transitionDelay: `${index * 60}ms` }}
            >
              <h3 className="font-display font-bold text-foreground text-base mb-2 group-hover:text-gold transition-colors">
                {item.title}
              </h3>
